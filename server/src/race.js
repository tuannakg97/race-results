...
async function getRaceData(years) {
  ...
  for await(const year of years) {
    ...
    const paginationURL = `https://www.formula1.com/en/results.html/${year}/races.html`
    ...
    await $("table > tbody > tr").each((index, element) => {
      ...
      const firstName = $$("td.dark.bold > span.hide-for-mobile").prop("innerHTML");
      const lastName = $$("td.dark.bold > span.hide-for-tablet").prop("innerHTML");
